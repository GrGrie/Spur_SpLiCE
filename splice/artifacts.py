...
def scratch_root() -> Path:
    """Return the cluster artifact root, allowing an explicit test/local override."""

    return Path(os.environ.get("SPUR_SPLICE_ARTIFACT_ROOT", DEFAULT_SCRATCH_ROOT)).expanduser()
...
def seed_run(seed: int, study: str, arm: str | None = None) -> Path:
    ...
    path = OUTPUT_ROOT / "seeds" / _safe_name(study, "study") / f"seed_{int(seed):02d}"
    return path / _safe_name(arm, "arm") if arm else path


def run_directory(seed: int, study: str, arm: str, attempt_id: str) -> Path:
    return seed_run(seed, study, arm) / _safe_name(attempt_id, "attempt")
...
def scratch_binary_directory(kind: str, identity: Mapping[str, Any]) -> Path:
    ...
    return (
        ...
        / kind
        / "Spur_SpLiCE"
        / _safe_name(str(identity["study"]), "study")
        / f"seed_{int(identity['seed']):02d}"
        / _safe_name(str(identity["arm"]), "arm")
        / _safe_name(str(identity["attempt_id"]), "attempt")
    )


def binary_destination(
    local_path: str | Path,
    payload_bytes: int,
    *,
    kind: str,
