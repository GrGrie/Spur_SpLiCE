...
The cluster runs on dgx001-dgx004, so the Linux snapshot is retaken there by that job when the phase 0 code and the current code agree. Golden training snapshots now record the CPU model they were taken on, and on another processor the comparison is skipped with a pointer to the bisection job instead of failing.
