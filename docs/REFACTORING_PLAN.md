...
Cluster verification of phases 1–8b: golden smoke job 23870258 (V100, AMP, channels-last, commit `8c88b20`) reproduces the phase 0 run 23823905 exactly for every SSL loss term and every probe accuracy of all five training modes; only the eigenvalue diagnostics (entropy, effective rank) and the probe gradient norms differ, in the seventh significant digit. On CPU the same code reproduces the phase 0 snapshot bit for bit, including those diagnostics. The dictionary check (job 23870257) passed: the Waterbirds cache from the code before phase 8b and from this code are equal in every field, and a file holding the Open Images words reproduces the bundled dictionary.

The CPU suite of the same job failed one test: the Linux golden training snapshot, taken on node `gpu014`, expects `cospro` `Average over 10 last linear val acc` = 75.0 and node `dgx002` gave 62.5. The probe accuracies of the 16-image fixture differ between machines for identical code (`cospro`: 75.0 on gpu014 CPU, 50.0 on Windows CPU, 50.0 on V100), so the snapshot is suspected to be node-specific. `scripts/bisect_golden_training.sbatch` runs the phase 0, phase 4, phase 8 and current code on one node and compares them bit for bit; its verdict decides the merge.
Bisection job 23871225 on dgx002 (Intel Xeon E5-2698 v4) ran the phase 0, phase 4, phase 8 and current code (`37d6096`, `34c75a0`, `8398090`, `f11d41e`): all four agree bit for bit on every SSL loss term and every probe metric of all five modes. The phase 0 code gives the same 62.5 on dgx002, so the failure came from the node, and the refactor did not change a single number. The Linux snapshot is now the phase 0 code's result on dgx002. Golden training snapshots now record the CPU model they were taken on, and on another processor the comparison is skipped with a pointer to the bisection job instead of failing.

Graphs behind the paper: the committed `outputs/shared/waterbirds/graphs/crp_graph.json` and `raw_clip_graph.json` (restored in `899c898`) have other byte fingerprints than the ones the paper runs recorded (`c5066726…`, `c81c93e6…`). Every property those runs recorded about their graph (artifact type, audit config, selected groups, removed concepts and degree statistics) equals the committed graphs for all 16 core runs, so the files hold the same graphs in another serialization. A retrained seed confirms it end to end.
