...
**Current tracks:** SpLiCE-CRP v3 (active branch architecture with legacy CoBalT),
SpLiCE-CRP v2 (historical compatibility), and SpLiCE-CQT v1 (experimental).
CRPv4 is the main-branch architecture and is intentionally absent here.
...
SSL training. Frozen OpenCLIP, SpLiCE, and CoBalT are teachers/auditors only; the
...
- fixed CoBalT VQ memberships and label-free assignment confidence when enabled;
...
unlabeled images
   ...
   └─ CoBalT ResNet + Slot Attention + VQ → independent concept memberships
                    │
       confidence-aware CRP or CQT frozen audit
                    │
...
the SpLiCE image mean, sparse SpLiCE codes, and the dictionary/vocabulary. The
complete Open Images V7 label vocabulary is the
...
3. **Find relations:** keep positive intervention-gain pairs with an unlabeled
   activation difference, reciprocal projected-CLIP neighbourhoods, and remaining
   SpLiCE-concept agreement.
...
The main branch's CRPv4 replaces the independent CoBalT/VQ concept system with a
CLIP-patch spatial branch mapped directly into the SpLiCE vocabulary. This CRPv3
branch deliberately does not contain that implementation. It exists to preserve
the pre-CRPv4 comparison in which CoBalT is an independent evaluator that changes
teacher-graph construction while SpLiCE keeps its own sparse named concepts.
...
6. Gate on cross-fold support/context, pseudo-state predictability, quotient
   efficacy, positive gain above random-contrast and shuffled-state nulls, word
   preservation, and coverage. Emit concept cards with words, metrics,
...
projection wins; or that graph KL transfers the desired invariance. These require
matched controls and multiple seeds.
...
Required pilot controls: SimCLR; raw-CLIP relational distillation; CRP; CQT;
shuffled concepts/states; matched random subspaces/contrasts; hard-positive CRP v1;
and Ghanooni spectral regularization.
...
| scripts/prepare_crp_splice_only_sweep.sbatch | no-CoBalT SpLiCE-only graph controls |
...
4. Treat raw-CLIP, shuffled, random, and spectral controls as required.
...
9. After a material method, loss, graph, protocol, reporting, or interpretation
   change, append a dated chronology entry to `Project History.md` with prior state,
   change, reason, consequences, track, and affected reporting snapshot. Do not
   rewrite history.

## 11. Immediate next work

1. Finish Waterbirds CRP/CQT frozen audits and post-hoc graph falsifiers.
2. Run raw-CLIP, random, shuffled, and spectral controls.
