"""Label-free CoSpRo graph distillation for the SimCLR student."""
...
from cospro.compat import LEGACY_TEACHER_GRAPH_ARTIFACTS
from cospro.pipeline.graph_io import graph_fingerprint, load_graph_json
...
def validate_teacher_graph(graph: dict, expected_sample_ids: Sequence[str] | None = None) -> dict:
    """Validate the sparse CoSpRo graph before it can influence training."""

    if not isinstance(graph, dict):
        raise ValueError("CoSpRo teacher graph must be a dictionary.")
    ...
    if forbidden:
        raise ValueError(f"CoSpRo teacher graph contains forbidden annotation keys: {sorted(forbidden)}")
    missing = REQUIRED_GRAPH_KEYS.difference(graph)
    if missing:
        raise ValueError(f"CoSpRo teacher graph is missing required keys: {sorted(missing)}")
    ...
    if not sample_ids or len(sample_ids) != len(set(sample_ids)):
        raise ValueError("CoSpRo graph sample_ids must be non-empty and unique.")
    if expected_sample_ids is not None and sample_ids != [str(value) for value in expected_sample_ids]:
        raise ValueError(
            "CoSpRo graph sample_ids do not exactly match the SSL train split. "
    ...
    if indices.ndim != 2 or weights.shape != indices.shape:
        raise ValueError("CoSpRo neighbor_indices and weights must be aligned rank-2 tensors.")
    if indices.shape[0] != n_samples or confidence.shape != (n_samples,):
        raise ValueError("CoSpRo graph tensors must contain one row per sample_id.")
    if not torch.isfinite(weights).all() or not torch.isfinite(confidence).all():
        raise ValueError("CoSpRo graph contains non-finite weights or confidence values.")

    valid = indices >= 0
    if torch.any(indices < -1) or torch.any(indices[valid] >= n_samples):
        raise ValueError("CoSpRo neighbor index is outside the sample_id range.")
    if torch.any(weights < 0) or torch.any(weights[~valid] != 0):
        raise ValueError("CoSpRo weights must be non-negative and zero for padded edges.")
    rows = torch.arange(n_samples).view(-1, 1).expand_as(indices)
    if torch.any(indices[valid] == rows[valid]):
        raise ValueError("CoSpRo teacher graph must not contain self-edges.")
    row_sums = weights.sum(dim=1)
    supported = row_sums > 0
    if torch.any(valid & (weights <= 0)):
        raise ValueError("Every valid CoSpRo edge must have positive weight.")
    if torch.any(~torch.isclose(row_sums[supported], torch.ones_like(row_sums[supported]), atol=1e-5)):
        raise ValueError("Supported CoSpRo graph rows must be row-stochastic.")
    if torch.any(valid.any(dim=1) != supported):
        raise ValueError("CoSpRo graph rows cannot contain zero-weight or weight-only edges.")
    ...
    if anchor_confidence.shape != (n_samples,) or not torch.isfinite(anchor_confidence).all():
        raise ValueError("CoSpRo anchor_confidence must contain one finite value per sample.")
    if torch.any(anchor_confidence < 0) or torch.any(anchor_confidence > 1 + 1e-6):
        raise ValueError("CoSpRo anchor_confidence must be in [0, 1].")
    if torch.any((anchor_confidence > 0) != supported):
        raise ValueError("CoSpRo anchor confidence support must match graph edge support.")
...
def load_teacher_graph(
    ...
    if not graph_path.is_file():
        raise FileNotFoundError(f"CoSpRo teacher graph not found: {graph_path}")
...
def build_cospro_concept_report(graph: dict) -> dict:
    ...
    return {
        "artifact": "cospro_concept_report_v1",
        "interpretation": (
            "teacher_projected means that the concept direction was removed only while "
            "constructing CoSpRo teacher relations; it does not assert literal erasure from "
...
class IndexedCoSpRoDataset(Dataset):
    ...
    def __init__(self, dataset) -> None:
        ...
        if self.source_indices is None or self.source_dataset is None or self.transform is None:
            raise ValueError("CoSpRo training requires an indexed subset with an image transform.")
        if not hasattr(self.source_dataset, "get_input"):
            raise ValueError("CoSpRo source dataset must expose get_input without annotations.")
...
class CoSpRoGraphBatchSampler(Sampler[list[int]]):
    ...
    def __init__(
        ...
        if batch_size < 2:
            raise ValueError("CoSpRo graph training requires batch_size >= 2.")
...
class CoSpRoRelationalRegularizer:
    ...
    def __init__(
        ...
        if weight < 0:
            raise ValueError("CoSpRo relational weight must be non-negative.")
        if temperature <= 0:
            raise ValueError("CoSpRo student temperature must be positive.")
        if start_epoch < 0 or warmup_epochs < 0:
            raise ValueError("CoSpRo start and warmup epochs must be non-negative.")
        if decay_start_epoch < 0 or decay_end_epoch < 0:
            raise ValueError("CoSpRo decay epochs must be non-negative.")
        if bool(decay_start_epoch) != bool(decay_end_epoch):
            raise ValueError("CoSpRo decay start/end must both be zero or both be set.")
        if decay_end_epoch and decay_end_epoch <= decay_start_epoch:
            raise ValueError("CoSpRo decay end must be greater than decay start.")
    ...
    def __call__(self, embeddings: torch.Tensor, sample_indices: torch.Tensor) -> torch.Tensor:
        if embeddings.ndim != 2 or embeddings.shape[0] % 2:
            raise ValueError("CoSpRo regularization expects two aligned views per batch sample.")
        batch_size = embeddings.shape[0] // 2
        sample_indices = torch.as_tensor(sample_indices).detach().long().cpu().view(-1)
        if sample_indices.shape != (batch_size,):
            raise ValueError("CoSpRo regularization requires one graph-row index per batch sample.")
