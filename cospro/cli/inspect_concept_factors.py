...
``--diagnose`` adds the post-hoc validity of the factors against the hidden class and spurious
attribute (``cospro.diagnostics.factor_validity``). It reads labels to evaluate, never to select.
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--merge-similarity", type=float, default=DEFAULTS["factor_merge_similarity"])
    ...
    parser.add_argument("--diagnose", action="store_true", help="Add the post-hoc validity against hidden labels.")
    parser.add_argument("--data-folder", default=os.environ.get("DATA_FOLDER", ""),
                        help="Dataset root for --diagnose; defaults to $DATA_FOLDER.")
...
def main(argv: list[str] | None = None) -> Path:
    ...
    if args.diagnose:
        if not args.data_folder:
            raise SystemExit("--diagnose needs --data-folder or $DATA_FOLDER.")
    ...
    if "validity" in report:
        print(format_diagnosis(report["validity"]))
