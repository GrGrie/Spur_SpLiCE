...
class ReproducibilityTests(unittest.TestCase):
    ...
    def test_cospro_resume_rejects_a_different_teacher_graph(self):
        model = torch.nn.Linear(2, 1)
        optimizer = torch.optim.SGD(model.parameters(), lr=0.1)
        with tempfile.TemporaryDirectory() as temporary_directory:
            checkpoint_path = str(Path(temporary_directory) / "cospro_resume.pth")
            save_checkpoint(
                model,
                optimizer,
                argparse.Namespace(cospro_graph_fingerprint="graph-a"),
                1,
                checkpoint_path,
            )
    ...
    def test_resume_reads_the_graph_fingerprint_of_a_crp_era_checkpoint(self):
        model = torch.nn.Linear(2, 1)
        optimizer = torch.optim.SGD(model.parameters(), lr=0.1)
        with tempfile.TemporaryDirectory() as temporary_directory:
            checkpoint_path = str(Path(temporary_directory) / "legacy_resume.pth")
            save_checkpoint(model, optimizer, argparse.Namespace(crp_graph_fingerprint="graph-a"), 1, checkpoint_path)
            self.assertEqual(
                load_checkpoint(model, optimizer, checkpoint_path, torch.device("cpu"),
                                expected_cospro_graph_fingerprint="graph-a"),
                1,
            )
            with self.assertRaisesRegex(ValueError, "different teacher graph"):
                load_checkpoint(model, optimizer, checkpoint_path, torch.device("cpu"),
