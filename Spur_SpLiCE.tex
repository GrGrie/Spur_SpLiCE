...
We also implement an optional spatial extension and a direct reconstruction
transfer control. The completed experiments characterize the ordinary CRP
pipeline; the latter extensions have no completed student results in the
available evidence.
...
support spurious-correlation mitigation \citep{arefin2024cobalt}. Our spatial
extension draws on concept balancing while expressing its image-specific
evidence in the fixed SpLiCE vocabulary. The main CRP experiment uses no CoBalT
memberships. These methods motivate the architecture and evaluation design;
...
SCLIP adapts the final attention layer for dense visual evidence \citep{wang2024sclip}.
...
concept support. Artifact confidence interpolates between the original and spatially weighted
codes. Row rescaling preserves each image's original total code mass whenever
the weighted row remains nonzero; a positive floor ensures this for every
originally nonzero row.
...
All completed student comparisons in this paper use ordinary SpLiCE codes.
...
\section{Current Extensions and Open Experiments}
The spatial extension in Section~\ref{sec:spatial} is implemented with separate artifacts for
patchwise and slot-based evidence. Its empirical contribution remains open.
A further implemented control tests whether the sparse reconstruction can be
transferred directly to the student. A separate projection head $g_\psi$ predicts
a frozen unit-normalized target $t_i$ using
\begin{equation}
 \mathcal{L}_{\mathrm{transfer}}=
 \frac{1}{2|B_V|}\sum_{i\in B_V}\sum_{v=1}^{2}
 \left[1-\cos\!\left(g_\psi(f_\theta(x_i^{(v)})),t_i\right)\right].
\end{equation}
Targets are centered raw CLIP embeddings, normalized SpLiCE reconstructions or
a fixed derangement of valid reconstructions. Here $B_V$ contains the valid
target rows in the batch; an empty $B_V$ contributes zero loss. The objective is
$\mathcal{L}_{\mathrm{SimCLR}}+\alpha(t)\mathcal{L}_{\mathrm{transfer}}$.
The matched SimCLR arm retains the same head with transfer weight zero.
This control asks whether correspondence with the sparse reconstruction matters
when the graph and graph-aware sampler are absent.

The configured follow-up uses seeds 1 and 3, 500 SSL epochs and maximum transfer
weight 0.1 after the same ten-epoch delay and warm-up. Its SimCLR temperature
is 0.5, so its primary comparison is its own matched baseline. Saved-probe
integrity and gradient diagnostics precede the full runs. No completed results
from this direct-transfer protocol or the spatial variants are included in the
available artifacts.
...
The current direct-transfer and spatial controls address parts of this problem
but have no completed student evaluation in this report.
