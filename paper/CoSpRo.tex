...
The worst-group advantage of CoSpRo holds across training (Figure~\ref{fig:curves}). Its smoothed mean worst-group accuracy leads all other methods at every probe from epoch 50 onward; from epoch 100 its margin over the semantic graph, the closest method, ranges from 0.8 to 3.0 points. Average accuracy separates later: CoSpRo leads at every probe from epoch 400 onward.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/training_curves.pdf}
\caption{Waterbirds validation accuracy during SSL training for the methods of Table~\ref{tab:semantic}. Each line is the mean over seeds 1--4 of the periodic linear probes, run every 25 epochs, smoothed with a three-point moving average. All graph methods use the KL relational loss. Top: worst-group accuracy. Bottom: average accuracy. Table~\ref{tab:semantic} reports the unsmoothed epoch-500 values.}
\label{fig:curves}
\end{figure}
...
Figure~\ref{fig:edges} shows retained edges. Removing the Bamboo, Sunset, Tawny owl or Monochrome photography subspace links a bird to a bird of the same class on the other background. Removing Glory of the snow links a landbird to a waterbird, an edge that transfers the wrong class.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/edge_examples.pdf}
\caption{Retained CoSpRo teacher edges on Waterbirds training images. Each column pairs an anchor (top) with its teacher neighbour (bottom); the column title names the concept whose subspace was removed when the edge was found. The four left pairs keep the bird class and change the background, the relations CoSpRo aims to add. They are the retained pairs of this kind with the largest projection gain, one per concept group. The dashed pair on the right is the heaviest retained wrong-class edge from another group. Class and background labels are shown for illustration and play no part in graph construction.}
\label{fig:edges}
\end{figure}

Post-hoc labels show both useful cross-context relations and target mismatches (Figure~\ref{fig:transitions}, Appendix~\ref{app:graphaudit}). For landbirds on water, same-target cross-background relations carry 59.22\% of CoSpRo's confidence-weighted graph mass versus 44.96\% for the raw graph. For waterbirds on land, the corresponding share barely changes (48.26\% versus 48.95\%), while wrong-target mass increases to 45.76\% from 39.73\%. These failures suggest that projection can't claim to simply remove spurious information, despite improving mean test WGA. Frozen-feature diagnostics confirm that the concept representation contains useful predictive information, but use different preprocessing and external pretraining; they are reported separately in Appendix~\ref{app:signal}.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/transition_matrices.pdf}
\caption{Teacher-edge mass between Waterbirds (class, background) groups. Each anchor contributes its confidence times its teacher preference, as in the CoSpRo objective, and each row sums to 100\%. Dashed cells keep the class and change the background. Left: raw CLIP graph. Middle: CoSpRo graph. Right: CoSpRo minus raw CLIP in percentage points. LB denotes landbird and WB waterbird. Labels are used only after graph construction.}
