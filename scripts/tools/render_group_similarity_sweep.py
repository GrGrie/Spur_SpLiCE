...
def _groups(graph: dict, cache: dict) -> list[Group]:
    ...
    if not output:
        raise ValueError("The graph contains no concept groups with concept indices.")
    return output
...
def generate_report(cache_path: Path, graph_path: Path, data_folder: Path, output_path: Path) -> Path:
...
def main() -> None:
    args = parse_args()
    output = generate_report(args.cache, args.graph, args.data_folder, args.output)
