...
def _evaluate_graph(graph_path: Path, rows: list[dict]) -> None:
    ...
    opposite_rate = same_target_opposite_spurious / same_target if same_target else 0.0
    print(f"graph={graph_path} edges={edge_count} anchor_coverage={supported.float().mean():.4f}")
    print(f"same_target_precision={precision:.4f} ({same_target}/{edge_count})")
    print(f"same_target_opposite_spurious_rate={opposite_rate:.4f} ({same_target_opposite_spurious}/{same_target})")
...
def main() -> int:
    ...
    parser.add_argument("--graph", required=True, type=Path, nargs="+")
    ...
    for graph_path in args.graph:
        _evaluate_graph(graph_path, rows)
    return 0
