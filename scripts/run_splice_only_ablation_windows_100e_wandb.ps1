...
function Invoke-CondaPython {
    ...
    # Windows PowerShell 5.1 can turn normal native stderr output (e.g. wandb)
    # into NativeCommandError when $ErrorActionPreference is Stop.  Let the
    # native process finish, then decide success from its real exit code.
...
if ([string]::IsNullOrWhiteSpace($CachePath)) {
    $SharedCachePath = Join-Path $RepoRoot `
        "outputs\windows_splice_only_ablation\waterbirds_train_features_oi_v7.pt"
...
# Preserve JSON double quotes through Windows PowerShell 5.1 -> conda.exe.
...
Invoke-CondaPython -Arguments @("-c", $GraphValidation, $GraphPath, $GraphConfigJsonNative)
Invoke-CondaPython -Arguments @(
...
if (Test-CompletedProbeLog $BaselineLog) {
    ...
    Invoke-CondaPython -Arguments ($CommonTrainingArguments + @(
        ...
        "--wandb_tags", "windows_local,screen,seed_$Seed,openimages_v7,baseline"
    )) -LogPath $BaselineLog
}
...
if (Test-CompletedProbeLog $SpliceLog) {
    ...
    Invoke-CondaPython -Arguments ($CommonTrainingArguments + @(
        ...
        "--wandb_tags", "windows_local,screen,seed_$Seed,openimages_v7,splice_only,no_cobalt"
    )) -LogPath $SpliceLog
}
...
if ($IncludeCobalt) {
    ...
    $CobaltGraphConfigJsonNative = $CobaltGraphConfigJson -replace '"', '\"'
    ...
    Invoke-CondaPython -Arguments @("-c", $GraphValidation, $CobaltGraphPath, $CobaltGraphConfigJsonNative)
    Invoke-CondaPython -Arguments @(
    ...
    if (Test-CompletedProbeLog $CobaltLog) {
        ...
        Invoke-CondaPython -Arguments ($CommonTrainingArguments + @(
            ...
            "--wandb_tags", "windows_local,screen,seed_$Seed,openimages_v7,cobalt,crpv3"
        )) -LogPath $CobaltLog
    }
}
