...
function Invoke-CondaPython {
    ...
    if ([string]::IsNullOrWhiteSpace($LogPath)) {
        ...
        # Windows PowerShell 5.1 turns native-process stderr into ErrorRecords.
        # With the script-wide $ErrorActionPreference = "Stop", harmless Python/Conda
        # stderr (warnings, progress, logging) can therefore abort this wrapper before
        # we get a chance to inspect the native process exit code.
